...
int		CXYChart::ShowPopupMenu( CMenu *passedMenu, CWnd *window, UINT nFlags, CPoint point )
{
	...
	gridSubMenu.AppendMenu( MF_STRING | flag, kMenuMajorHorizGrids, "Major Horizontal Grids" );
	flag = GetPopupCheckedFlag(this->m_UseMajorVerticalGrids);
	gridSubMenu.AppendMenu( MF_STRING | flag, kMenuMajorVertGrids, "Major Vertical Grids" );
	flag = GetPopupCheckedFlag(this->m_UseMinorHorizontalGrids);
	gridSubMenu.AppendMenu( MF_STRING | flag, kMenuMinorHorizGrids, "Minor Horizontal Grids" );
	flag = GetPopupCheckedFlag(this->m_UseMinorVerticalGrids);
	gridSubMenu.AppendMenu( MF_STRING | flag, kMenuMinorVertGrids, "Minor Vertical Grids" );
	menuPtr->AppendMenu( MF_POPUP | MF_STRING, (UINT) gridSubMenu.GetSafeHmenu(), "Grids" );
	}

	// Separator
	menuPtr->AppendMenu( MF_SEPARATOR, 0, (LPCSTR)0 );
	...
	// Settings...
	menuPtr->AppendMenu( MF_STRING, kMenuPlotSettings, "Settings..." );
...
void	CXYChart::AddDataPopupMenus( int whichDataSet, CMenu *menu, int startVal )
{
	...
	dataMenu.AppendMenu( MF_SEPARATOR, 0, (LPCSTR)0 );
	...
	if( string == "" )
	{
		string.Format( "Data set %d of %d", whichDataSet + 1, GetDataSetCount() );
	}
	else
	{
	...
	// Add it to main menu
	dataMenu.AppendMenu( MF_SEPARATOR, 0, (LPCSTR)0 );
	dataMenu.AppendMenu( MF_STRING, startVal+kMenuDataRange-1, "Settings..." );
...
void	CXYChart::AddChartTypePopup( CMenu *addMenu, LPCSTR titleString, long compare, int startVal )
{
	...
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYChartPlain, "XY Scatter" );
	flag = GetPopupCheckedFlag( compare == kXYChartConnect );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYChartConnect, "XY Line" );
	flag = GetPopupCheckedFlag( compare == kXYChartHistogram );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYChartHistogram, "Histogram" );
}
...
void	CXYChart::AddMarkerPopup( CMenu *addMenu, LPCSTR titleString, int dataID, int startVal )
{
	...
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYMarkerNone, "No marker" );
	flag = GetPopupCheckedFlag( markerShape == kXYMarkerCircle );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYMarkerCircle, "Circle marker" );
	flag = GetPopupCheckedFlag( markerShape == kXYMarkerSquare );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYMarkerSquare, "Square marker" );
	flag = GetPopupCheckedFlag( markerShape == kXYMarkerTriangle );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYMarkerTriangle, "Triangle marker" );
	flag = GetPopupCheckedFlag( markerShape == kXYMarkerX );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYMarkerX, "X marker" );

	addMenu->AppendMenu( MF_SEPARATOR, 0, (LPCSTR)0 );

	flag = GetPopupCheckedFlag( GetMarkerFillState( dataID ) );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kMenuMarkerFill, "Filled marker" );
	...
	addMenu->AppendMenu( MF_POPUP, (UINT) subMenu.GetSafeHmenu(), "Marker size" );

	// Marker frequency
	freq = abs(GetMarkerFrequency( dataID ));
	if( freq == 1 ) freq = 10;
	flag = GetPopupCheckedFlag( GetMarkerFrequency( dataID ) == 1 );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kMenuMarkerFrequency, "Mark every point" );
	flag = GetPopupCheckedFlag( (GetMarkerFrequency( dataID )) > 1 );
	string.Format( "Mark every %d points", freq );
	...
	string.Format( "Mark %d points", freq );
...
void	CXYChart::AddLineStylePopup( CMenu *addMenu, LPCSTR titleString, int dataID, int startVal )
{
	...
	subMenu.AppendMenu( MF_STRING | flag, startVal,   "Solid ____________" );
	flag = GetPopupCheckedFlag( GetDataLineStyle( dataID ) == 1 );
	subMenu.AppendMenu( MF_STRING | flag, startVal+1, "Dashed _ _ _ _ _ _" );
	flag = GetPopupCheckedFlag( GetDataLineStyle( dataID ) == 2 );
	subMenu.AppendMenu( MF_STRING | flag, startVal+2, "Dotted . . . . . ." );
	flag = GetPopupCheckedFlag( GetDataLineStyle( dataID ) == 3 );
	subMenu.AppendMenu( MF_STRING | flag, startVal+3, "Dash-Dot _ . _ . _" );
	flag = GetPopupCheckedFlag( GetDataLineStyle( dataID ) == 4 );
	subMenu.AppendMenu( MF_STRING | flag, startVal+4, "Dash-Dot-Dot _ . ." );
