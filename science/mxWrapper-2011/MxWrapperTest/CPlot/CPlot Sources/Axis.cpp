...
void	CAxis::AddAxisPopupMenus( CMenu *menu, int startVal )
{
	...
	axisMenu.AppendMenu( MF_STRING | flag, startVal, "Auto range" );
	...
	axisMenu.AppendMenu( MF_SEPARATOR, 0, (LPCSTR)0 );
	...
	axisMenu.AppendMenu( MF_STRING, startVal+kMenuAxisRange-1, "Settings..." );
...
void	CAxis::AddNumberOfTicksPopup( CMenu *addMenu, LPCSTR titleString, long compare, int startVal )
{
	...
	subMenu.AppendMenu( MF_STRING | flag, startVal, "Auto" );

	for( i = 0; i < nTickSizes; i++ )
	{
		flag = GetPopupCheckedFlag( compare == tickSizes[i] );
		string.Format( "%d", tickSizes[i] );
